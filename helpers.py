...
def get_batch(data, batch_size=24*2):
    batch = data.sample(int(batch_size/2))
    img_paths = np.concatenate((batch['center'].str.strip().values,
                               batch['center_flipped'].str.strip().values))
    steerings = np.concatenate((batch['steer'].values,
                               batch['steer_flip'].values))
    img_list = []
...
def train_sample_generator(train_df, batch_size=24*2):
    """
...
def valid_sample_generator(valid_df, batch_size_valid=24*2):
